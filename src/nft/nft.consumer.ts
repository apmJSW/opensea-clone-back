...
import { Nft, NftContract, NftProperty } from '../entities';
  ...
  async loadTokenList(job: Job) {
    ...
    while (startToken) {
      ...
      const nftRows = nfts.map((nft) => {
        ...
        const attributes = nft?.metadata?.attribute;
        nftRow.properties = Array.isArray(attributes)
      ...
      this.logger.log(
        `lodading NFTs for contract ${job.data.contractAddress}. (${added}/?)`,
      );
    }
